...
def compile_pdf_resume(profile: dict, output_pdf_path: str) -> str:
    """
    Compile candidate profile into an executive ATS-formatted PDF guaranteed to fit on 1 PAGE.
    """
    ...
    COLOR_BODY    = (40, 40, 40)     # Dark Charcoal
    COLOR_LINK    = (15, 75, 165)    # Link Blue
    ...
    for idx, item in enumerate(c_items):
        ...
        if link_url:
            ...
            pdf.write(4.5, _clean_str(disp_text), link=_clean_str(link_url))
        else:
            pdf.set_font("Helvetica", "", 8.5)
            pdf.set_text_color(*COLOR_BODY)
            pdf.write(4.5, _clean_str(disp_text))
    ...
    def add_section_header(header_text):
        ...
        pdf.cell(0, 5, header_text.upper(), new_x="LMARGIN", new_y="NEXT")
